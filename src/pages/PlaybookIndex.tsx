...
export default function PlaybookIndex() {
  return (
    <>
      <Seo
        title="My Product Thinking — Aryan Panwar"
        description="How I apply PM frameworks — JTBD, RICE, opportunity trees, north-star metrics — to real projects like FitWardrobe, Mithivoices, and SEO-GEO Optimizer."
        path="/thinking"
        image={portrait}
        keywords={["Product Thinking", "JTBD", "RICE", "North Star Metric", "Opportunity Trees", "PM Frameworks"]}
        jsonLd={{
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border">
            {playbook.map((p) => (
              <motion.div
                ...
                className="[&:nth-child(-n+3)]:md:border-b [&:nth-child(-n+3)]:md:border-border"
              >
                <Link to={`/thinking/${p.slug}`} className="block p-8 h-full group hover:bg-background/60 transition-colors">
